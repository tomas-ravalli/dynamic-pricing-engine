...
def generate_synthetic_data(num_matches=50):
    ...
    This version includes more complex interactions between features to better simulate
    real-world scenarios. Each row represents a specific seating zone for a given match.
    ...
    for i in range(num_matches):
        ...
        # Correlate EA strength with opponent tier for realism
        ...
        team_position = np.random.randint(1, 5) # Assume the team is always a top contender
        weekday_match = np.random.choice([True, False], p=[0.3, 0.7])
        top_player_injured = np.random.choice([True, False], p=[0.15, 0.85])
        # League winner is only known towards the end of a "season"
        ...
        # --- Create a unified "Match Excitement Factor" ---
        # This factor will drive most demand signals.
        excitement = tier_multiplier
        excitement -= (team_position - 1) * 0.05  # Higher position = more excitement
        ...
        excitement = np.clip(excitement, 0.5, 2.0) # Bound the excitement factor

        # --- Simulate external signals based on excitement ---
        base_buzz = excitement + (90 - days_until_match) / 90.0
        
        social_media_sentiment = np.clip(np.random.normal(0.2, 0.4) * base_buzz, -1.0, 1.0)
        google_trends_index = int(np.clip(np.random.randint(30, 80) * base_buzz, 20, 100))
        internal_search_trends = int(np.clip(np.random.randint(500, 2000) * base_buzz, 100, 5000))
        web_visits = int(np.clip(np.random.randint(10000, 50000) * base_buzz, 5000, 100000))
        web_conversion_rate = np.clip(np.random.normal(0.02, 0.01) * base_buzz, 0.005, 0.1)
        flights_to_barcelona_index = int(np.clip(np.random.randint(40, 90) * base_buzz, 20, 100))
        
        # --- Generate data for each zone in the match ---
        for zone_name, zone_info in zones.items():
            # Simulate zone-specific sales based on excitement and zone properties
            demand_base = zone_info['demand_factor'] * excitement * ((90 - days_until_match) / 90.0)**0.5
            demand_noise = np.random.normal(1.0, 0.1) # Add some random variance
            
            # Calculate historical sales as a % of capacity
            sales_percentage = np.clip(demand_base * demand_noise, 0.1, 0.95)
            zone_historical_sales = int(zone_info['capacity'] * sales_percentage)
            ...
            # Simulate pricing
            price_multiplier = excitement + (1 - ticket_availability_pct) # Price goes up as seats fill up
            ticket_price = round(zone_info['base_price'] * price_multiplier * np.random.normal(1.0, 0.05), 2)
            competitor_avg_price = round(ticket_price * np.random.uniform(0.8, 1.2), 2) # Competitor price is related but varies
...
def main():
    ...
    synthetic_df = generate_synthetic_data(num_matches=50)
